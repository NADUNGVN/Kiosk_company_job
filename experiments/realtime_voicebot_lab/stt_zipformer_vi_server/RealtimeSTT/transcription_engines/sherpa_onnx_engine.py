...
DEFAULT_SHERPA_ONNX_ZIPFORMER_VI_MODEL = "sherpa-onnx-zipformer-vi-30M-int8-2026-02-09"
...
KNOWN_MODEL_DIRS = {
    ...
    "hynt/Zipformer-30M-RNNT-6000h": DEFAULT_SHERPA_ONNX_ZIPFORMER_VI_MODEL,
    DEFAULT_SHERPA_ONNX_ZIPFORMER_VI_MODEL: DEFAULT_SHERPA_ONNX_ZIPFORMER_VI_MODEL,
}
...
class SherpaOnnxZipformerBackend(SherpaOnnxOfflineBackend):
    family = "sherpa_onnx_zipformer"
    default_model_dir = DEFAULT_SHERPA_ONNX_ZIPFORMER_VI_MODEL
    download_url = ZIPFORMER_VI_DOWNLOAD_URL
    ...
    def _create_recognizer(self, recognizer_cls):
        ...
        kwargs.update(
            {
                "encoder": self._file("encoder", "encoder.int8.onnx"),
                "decoder": self._file("decoder", "decoder.onnx"),
                "joiner": self._file("joiner", "joiner.int8.onnx"),
                "tokens": self._file("tokens", "tokens.txt"),
                "sample_rate": _int_option(self.engine_options, "sample_rate", 16000),
                "feature_dim": _int_option(self.engine_options, "feature_dim", 80),
        ...
        bpe_vocab = self.engine_options.get("bpe_vocab")
        if bpe_vocab is None and (self.model_dir / "bpe.model").is_file():
            bpe_vocab = "bpe.model"
        if bpe_vocab:
            kwargs["bpe_vocab"] = str(_maybe_join(self.model_dir, bpe_vocab))

        if self.engine_options.get("model_type"):
            kwargs["model_type"] = self.engine_options["model_type"]
...
class SherpaOnnxZipformerEngine(BaseTranscriptionEngine):
    engine_name = "sherpa_onnx_zipformer"

    def __init__(self, config, backend=None, backend_cls=None):
        super().__init__(config)
        self.backend = backend or (backend_cls or SherpaOnnxZipformerBackend)(config)
    ...
    def transcribe(self, audio, language=None, use_prompt=True):
        ...
        detected_language = (
            output.language
            or language
            or (self.config.engine_options or {}).get("language")
            or "vi"
        )
        return TranscriptionResult(
            text=output.text,
