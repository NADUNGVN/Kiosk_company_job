...
| **Giai Đoạn 5: Voicebot - Laptop TTS Integration** | Tích hợp Windows SAPI TTS phát loa trực tiếp tại Laptop, áp dụng cơ chế gom câu (Sentence Buffering) & ngắt lời chủ động (Active Interruption) từ repo RealtimeVoiceChat. | **Laptop**: Server STT giải mã văn bản, truyền từng token vào bộ đệm câu và đẩy vào hàng đợi Windows TTS để đọc loa Laptop.<br>Khi có câu hỏi mới, loa tự động ngắt lập tức. | - Trình đọc SAPI hoạt động ổn định trên thread ẩn của Laptop.<br>- Cơ chế ngắt lời (Interruption) và phát cuốn chiếu (Sentence-buffered) hoạt động mượt mà, trơn tru. | **Hiện Tại (Đang Test)** |
